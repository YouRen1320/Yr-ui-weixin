...
Page({
  ...
  onReady:function(){
    pvRef = this.selectComponent('#pvRef')
  },
  //rpx转化为px
  getPx(value) {
    let val = parseInt(wx.yrUi.rpx2px(Number(value)))
  ...
  init(e) {
    //裁剪图片宽度，固定为44（px）
    let cutGraphWidth = e.detail.cutGraphWidth
    this.setData({
      width:300,
      height:150
    })
  ...
  getVerifyImage() {
    ...
    this.setData({
       //带黑块的背景图，后端接口提供
      src:`${this.data.resUrl}/component/sc/bg_img_verify.png`,
      //裁剪下的图片，后端接口提供
      cutSrc:`${this.data.resUrl}/component/sc/cut_img.png`,
  ...
  verify(e) {
    ...
    //滑动距离
    let slipDistance = e.detail.slipDistance
    //验证方法，参考js方法自行转为后端方法
    //x1为裁剪黑块的left值，该值仅后端知道，示例使用的预处理数据，此处仅为演示
    let x1 = 222;
    //width为需要滑动的总距离
    let width = x1 - this.data.x
    //误差范围，后端自行定义（px）
    let range = 3
