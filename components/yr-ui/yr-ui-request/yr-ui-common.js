...
const base = {
  ...
  config() {
    return {
      //接口域名，如：https://yr-ui.cn（如果host为空，则直接使用传入的目标地址url）
      host: '',
      // 接口地址：/order/getList（如果host为空，直接传入：https://yr-ui.cn/order/getList）
      url: '',
      //参数
      data: {},
      //请求头
      ...
      brief: false,
      //String 请求标记，用于中断该请求，不同请求不可重复，只可包含数字、字母、下划线，如：yr-ui_001
      cancelToken: '',
      showLoading: true,
  ...
  getOptions(config) {
    ...
    const keys = ['host', 'timeout', 'prevent', 'keys', 'brief', 'cancelToken', 'showLoading', 'loadingText', 'errorMsg', 'arrayFormat'];
    keys.forEach(item => {
      delete options[item];
    })
    return options;
