...
Component({
  ...
  relations: {
    '../yr-ui-waterfall-item/yr-ui-waterfall-item': {
      type: 'descendant'
    }
  },
  data: {
  ...
  methods: {
    ...
    startSorting() {
      ...
      if (clen === llen && llen > 0) {
        let itemGap = 0
        if (this.data.isBefore) {
          this.data.leftHeight = 0
          this.data.rightHeight = 0
          this.data.loadedChild = this.data.childrenArr.concat(this.data.loadedChild)
        } else {
          this.data.loadedChild = this.data.loadedChild.concat(this.data.childrenArr)
        }
        const usageData = this.data.isBefore ? this.data.loadedChild : this.data.childrenArr;
