...
Component({
  ...
  methods: {
    ...
    getValueToDate(val) {
      ...
      if (type == 8)
        val = '00:' + val;
      let format = String(val).replace(/\-/g, '/');
    ...
    initData(values) {
      ...
      if (type < 6) {
        this.getYears()
        index = this.getIndex(this.data.years, values[0]);
        if (type > 1) {
          this.getMonths(index)
          idx = this.getIndex(this.data.months, values[1]);
      ...
      setTimeout(() => {
        let di = this.getIndex(this.data.days, values[2]);
        let hi = this.getIndex(this.data.hours, values[3]);
        let mi = this.getIndex(this.data.minutes, values[4])
        let si = this.getIndex(this.data.seconds, values[5])
        let vals = [
          [index],
          [index, idx],
          [index, idx, di],
    ...
    btnConfirm(e) {
      if (this.data.range) {
        ...
        if (!start) {
          if (this.data.isAuto) {
            ...
            if (!end) {
              ...
              this.toast(`请选择${this.data.end}`)
              return;
            } else {
              start = this.data.startDate.result;
            }
          } else {
            this.toast(`请选择${this.data.start}`)
            return;
          }
        } else if (start && !end) {
          if (this.data.isAuto) {
        ...
        if (!this.compareDate(start, end)) {
          this.toast(`${this.data.end}不能小于${this.data.start}`)
          return;
        }
      }
      setTimeout(() => {
    ...
    rangeChange(e) {
      ...
      let values = []
      if (type == 1) {
        const start = this.data.startDate.result;
        if (start) values = this.getValueToDate(start);
      } else {
        const end = this.data.endDate.result;
